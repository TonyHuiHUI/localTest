...
public class  GCMonitoring {
    ...
    public static void init() {
        ...
        for (GarbageCollectorMXBean gcBean : gcBeans) {
            System.out.println("register " + gcBean.getName() + " for " + Arrays.deepToString(gcBean.getMemoryPoolNames()));
            ...
            NotificationListener listener = new NotificationListener() {
                ...
                public void handleNotification(Notification notification, Object handback) {
                    ...
                    for (Map.Entry<String, MemoryUsage> entry : afterUsageMap.entrySet()) {
                        ...
                        usage.append("used:").append(handBack.handUsage(beforeUsage.getUsed(), afterUsage.getUsed(), beforeUsage.getMax())).append("; ");
                        usage.append("committed: ").append(handBack.handUsage(beforeUsage.getCommitted(), afterUsage.getCommitted(), beforeUsage.getMax()));

                        System.out.println(usage.toString());
                    }
                    totalGcTimeSpend += info.getGcInfo().getDuration();
                    //summary
                    long percent = (info.getGcInfo().getEndTime() - totalGcTimeSpend) * 1000L / info.getGcInfo().getEndTime();
